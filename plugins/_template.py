"""
Drop-in NEXUS AI plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — NEXUS AI discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the NexusUI instance — use player.write_log(f"NEXUS: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"NEXUS: {result_text}")
        except Exception:
            pass
    return result_text
