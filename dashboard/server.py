"""
dashboard/server.py — NEXUS AI Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Nexus Uploads",
        Path.home() / "Documents" / "Nexus Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Nexus AI Dashboard Port {port}"
        prog_rule  = "Nexus AI Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="nexus_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until NEXUS AI is run as Administrator.")
...
<p>Connecting to NEXUS AI…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in NEXUS AI UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
