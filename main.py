...
from ui import JarvisUI, NexusUI
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Nexus AI, an advanced and intelligent AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Nexus AI checks these topics once a day and alerts the user when there is a new development. "
...
class NexusLive:
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[NEXUS] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "Nexus AI").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "Nexus AI"
            _user_name = ""
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name in ("shutdown_nexus", "shutdown_jarvis"):
        ...
        print(f"[NEXUS] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[NEXUS] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[NEXUS] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[NEXUS] 🔗 Session resumption armed")
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[NEXUS] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    if self._wake_enabled:
                        ...
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: NEXUS online — sleeping. Say 'Hey Jarvis' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: NEXUS online.")
                ...
                if _is_reconnect_signal(e):
                    print("[NEXUS] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[NEXUS] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[NEXUS] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[NEXUS] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[NEXUS] Reconnecting in {delay}s...")
